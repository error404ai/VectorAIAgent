PROVIDER_TEMPLATE = """
console.log('[AutoWallet] Starting Solana wallet injection...');

if (window.solana && window.solana.isAutoWallet) {
    console.log('[AutoWallet] Provider already exists');
} else {
    console.log('[AutoWallet] Injecting provider...');
            ...
            try{
                if (crypto && crypto.subtle && crypto.subtle.importKey && crypto.subtle.sign){
                    ...
                    try{
                        ...
                        console.log('[AutoWallet] WebCrypto JWK path used');
    ...
    async function signMessage(message, opts) {
        try {
            ...
            console.error('[AutoWallet] Ed25519 signing failed, falling back to SHA-512:', e);
            const msgBuf = normalizeMessageInput(message, opts && (opts.encoding || opts.display));
    ...
    async connect(opts){ console.log('[AutoWallet] connect()', opts); this._publicKey = publicKeyObj; this._connected = true; window.dispatchEvent(new CustomEvent('solana#connect',{detail:{publicKey:this._publicKey}})); try{ this.emit('connect', this._publicKey); }catch(e){console.error(e);} window.dispatchEvent(new CustomEvent('phantom#connect',{detail:{publicKey:this._publicKey}})); return { publicKey: this._publicKey }; },
    async disconnect(){ console.log('[AutoWallet] disconnect()'); this._publicKey = null; this._connected = false; window.dispatchEvent(new CustomEvent('solana#disconnect')); try{ this.emit('disconnect'); }catch(e){console.error(e);} return; },
        async signMessage(message, opts){ console.log('[AutoWallet] signMessage()'); if(!this._connected) await this.connect(); const sigObj = await signMessage(message, opts); console.log('[AutoWallet] Message signed'); return { signature: sigObj.signature, publicKey: publicKeyObj, signatureBase58: sigObj.signatureBase58 }; },
        async signTransaction(transaction){ console.log('[AutoWallet] signTransaction()'); if(!this._connected) await this.connect(); const signed = await signTransactionInternal(transaction); return signed; },
        async signAndSendTransaction(transaction, options){ console.log('[AutoWallet] signAndSendTransaction()'); if(!this._connected) await this.connect(); const signed = await signTransactionInternal(transaction); try{ const sigStr = await sendRawTransactionInternal(signed, options); console.log('[AutoWallet] Transaction sent:', sigStr); return { signature: sigStr, publicKey: publicKeyObj }; } catch(e){ console.error('[AutoWallet] sendRawTransaction failed', e); throw e; } },
        async request(args){
            console.log('[AutoWallet] request() called with:', args);
        ...
        window.fetch = async function(input, init){
            ...
            try{
                ...
                if (res && res.status === 401 && url.startsWith(PUMP_API) && /\\/auth\\/(login|my-profile)/.test(url)){
                    console.log('[AutoWallet] pump.fun 401 detected; attempting challenge login');
                    // Debug: log a small snippet of the 401 body to understand expected schema
                    try {
                        const ct = res.headers.get('content-type') || '';
                        if (ct.includes('application/json')) {
                            const j = await res.clone().json().catch(()=>null);
                            console.log('[AutoWallet] 401 body (json):', j);
                        } else {
                            const t = await res.clone().text().catch(()=>null);
                            console.log('[AutoWallet] 401 body (text):', t && t.slice(0, 500));
                        }
                    } catch (e) {
                        console.log('[AutoWallet] Could not read 401 body');
                    }
                    try{
                        ...
                        if (ok){
                            console.log('[AutoWallet] pump.fun auth succeeded; retrying original request');
    ...
    console.log('[AutoWallet] Provider injected successfully');
    console.log('[AutoWallet] Public key:', pubkey);
}
"""
